describe Wirecard::Elastic::Transaction do
  ...
  subject(:creditcard_valid_response) { Wirecard::Elastic::Transaction.new(MERCHANT_CREDITCARD, TRANSACTION_CREDITCARD, PAYMENT_METHOD_CREDITCARD).response }
  subject(:upop_valid_response) { Wirecard::Elastic::Transaction.new(MERCHANT_UPOP, TRANSACTION_UPOP, PAYMENT_METHOD_UPOP).response }
  subject(:upop_invalid_response) { Wirecard::Elastic::Transaction.new(MERCHANT_UPOP, "wrong", PAYMENT_METHOD_UPOP).response }
